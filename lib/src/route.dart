...
abstract class Routes {
  static MaterialPageRoute materialRoutes(RouteSettings settings){
    switch(settings.name){
      ...
      if(settings.name.contains("/editproduct/")){return MaterialPageRoute(builder: (context) => EditProduct(productId: routeArray[2]));}
      return MaterialPageRoute(builder: (context) => Login());
    }
  }
  ...
  static CupertinoPageRoute cupertinoRoutes(RouteSettings settings){
    switch(settings.name){
      ...
      default:
      var routeArray = settings.name.split("/");
      if(settings.name.contains("/editproduct/")){return CupertinoPageRoute(builder: (context) => EditProduct(productId: routeArray[2]));}
      return CupertinoPageRoute(builder: (context) => Login());
    }
  }
}